...
class Course extends Admin_Controller
{
    ...
    public function add($id = false)
    {
        ...
        $this->data['course_cat'] = $this->db->get('cr_category')->result();
    ...
    {
        if ($id > 0) {
            $this->Course_model->delete($id, 'cr_category');
    ...
    public function category()
    {
        ...
        $this->data['course'] = $this->db->get_where('cr_category', array('status' => 1))->result();
        $this->load->view(admin_view('default'), $this->data);
    }
    ...
    public function category_add($id = false)
    {
        ...
        $this->data['course'] = $this->Course_model->getNew('cr_category');

        if ($id) {
            $this->data['course'] = $course = $this->Course_model->getRow($id, 'cr_category');
        ...
        $this->form_validation->set_rules('frm[name]', 'Category title', 'required');
        ...
        if ($this->form_validation->run()) {
            ...
            $slug = strtolower(url_title($slug));
            $formdata['slug'] = $this->Cms_model->get_unique_url($slug, $id);
            $id = $this->Course_model->save($formdata, 'cr_category');
            //echo $this->db->last_query();die();
