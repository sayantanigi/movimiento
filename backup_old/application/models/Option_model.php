...
class Option_model extends Master_model{
	...
	function all_options(){
		$data = array();
		$rest = $this -> db -> get('setting') -> result_array();
    ...
    function get_option($option_name){
        $d = $this -> db -> get_where('setting', array('option_name' => $option_name)) -> first_row('array');
        if(is_array($d)){
            return $d;
        }else{
	...
	function save_option($data){
		$rest = $this -> db -> get_where('setting', array('option_name' => $data['option_name']));
		if($rest -> num_rows() > 0){
