...
$.fn.sleekslider = function(object) {
	...
	if (object.thumbs != undefined || object.thumbs != null) {
		if (object.thumbs.length > 1) {
			...
			$prevThumbnail.attr('src', 'images/banner-v3/' + thumbs[thumbs.length - 1]);
			$prevText.text(labels[thumbs.length - 1]);

			$nextThumbnail.attr('src', 'images/banner-v3/' + thumbs[active]);
...
function animateSlider() {
	...
	$('.sleekslider-pagination > span.current').removeClass('current');
	$('.sleekslider-pagination > span:nth-child(' + next + ')').addClass('current');

	$('.tabs li.current').removeClass('current');
	$('.tabs li:nth-child(' + next + ')').addClass('current');
	...
	if (thumbs != undefined || thumbs != null) {
		if (prevClicked) {
			...
			$prevThumbnail.attr('src', 'images/banner-v3/' + thumbs[value]);
			$prevText.text(labels[value]);
		} else {
			$prevThumbnail.attr('src', 'images/banner-v3/' + thumbs[active - 1]);
			$prevText.text(labels[active - 1]);
		}

		if (active === (thumbs.length - 1)) {
			$nextThumbnail.attr('src', 'images/banner-v3/' + thumbs[0]);
			$nextText.text(labels[0]);
		} else {
			$nextThumbnail.attr('src', 'images/banner-v3/' + thumbs[next]);
...
$('.sleekslider-pagination > span').hover(function(){
	clearInterval(interval);
}, function() {
	interval = setInterval('cycleSlides()', speed);
});

$('.sleekslider-pagination > span').click(function () {
	if ($(".sleekslider > .slide").is(":animated")) {
		return false;
	} else {
